		...
		<div class="field">
			...
			<select id="select_indicadores" class="ui fluid search dropdown" >
				...
				<?php foreach ($this->indicadores_model->cargar("indicadores_activos", NULL) as $indicador) { ?>
					<option value="<?php echo $indicador->Pk_Id; ?>" data-familia="<?php echo $indicador->Fk_Id_Indicador_Familia; ?>"><?php echo "$indicador->Identificador - $indicador->Nombre"; ?></option>
				<?php } // foreach ?>
			</select>
		</div>
	</div><!-- Indicador -->
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("#select_unidad_funcional, #select_indicadores").on("change", function(){
			...
			id_familia = $("#select_indicadores option:selected").attr("data-familia");

			// Cuando todos los selects tengan un valor seleccionado
			if (id_unidad_funcional.val() != "" && id_indicador.val() != "") {
				// Se carga la interfaz
				cargar_interfaz("cont_segmentos", "<?php echo site_url('indicadores/cargar_interfaz'); ?>", {"tipo": "evaluaciones_familia", "id_unidad_funcional": id_unidad_funcional.val(), "id_indicador": id_indicador.val(), "id_familia": id_familia});
			} // if
		}); // change
